...
- **轮询**: 单全局轮询，每 3–5 秒拉第一页（`limit=500`），从最新往回走，遇到"已见过键"即停。**MVP 只拉单页**；当返回拉满整页（`length === limit`）时打 `console.warn` 提示可能溢页。完整的"首页全新则继续 `offset` 翻页"突发保护属 **P2+**（极端行情下一轮 >500 笔才会触发）。
