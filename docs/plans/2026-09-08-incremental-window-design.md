...
延迟地板 = 上游索引延迟,**2026-09-09 实测**(每 3s 探一次全量成交流最新行
年龄,30 样本):min 23s / 中位 71s / p90 109s / max 116s —— ~23s 固定管道
延迟 + ~90s 批量入索引周期(推断自年龄摆动区间)。据此校准:提频后
formation→emitted **中位 ~110–130s、p90 ~200s**(索引 ~70 + 相位 ~45 +
轮内 ~5–10),较基线 329s 改善 ~2.5–3 倍;旧基线分解闭环(~150 相位 +
~70 索引 + ~60–100 旧轮内垫时 ≈ 300 ✓)。下游按 formationTs 的新鲜度阈值
至少应容 ~200s + 投递 30s。注:评审附录 A 的 alerts 表测法不可用 ——
created_at 写的是成交自身时间戳(alertEngine.ts recordAlert),恒等于零。
稳态传输 ~1030 行/5min → ~50–100 行/90s 轮;热点日抓取成本与密度解耦
(单轮新增 <800 行内恒 1–8 页)。
...
- 共识 TG 升级推送可能更碎(5 分钟内 3→5 人原来合并一条,现在可能 3→4、4→5
  两条);
- cycle_metrics 写入 288 → 960 行/天(86400/90;无保留清理,~30 MB/年,暂可接受)。
...
2. **迟到索引(4.2,采纳)**:边距 180s→600s,静默窗口压到「迟到 >10 分钟
   才踩」;成本常态 ~36 行重叠。每小时重扫暂不加密 —— 实测(见上)成交可见时时间戳已老 23–116s,600s 边距余量充足。
