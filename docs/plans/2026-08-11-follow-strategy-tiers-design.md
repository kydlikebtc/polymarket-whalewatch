...
| D8  | C2「分歧解除」判据       | **少数边开始净卖** → 实施中修订为**已实现现金流口径** |

**D8 的修订（2026-08-11 实施中）：** 「少数边开始净卖」最初按 `signedNetUsd = Σ netShares × avgBuyPrice < 0` 实现，审查用真实代码复现了两个致命缺陷：

1. **对最自然的割肉形态失明** —— 钱包本窗口零买入、只把窗口前建的仓全卖光时 `avgBuyPrice = 0`，贡献恒为 `0` 而非负数。而"割肉离场不会顺手再买一点建仓价参照"恰恰是认输最典型的样子。
2. **方向会反** —— 买 100@0.5、卖 150@0.9 这种**赚钱的止盈**贡献 −25，被判成认输。

修订后的判据（用户裁决）：

- **少数边认输**：`sellUsd > buyUsd` 且（`buyShares === 0` 或 卖均价 < 买均价）
- **主导边**：`buyUsd > sellUsd` 且净投入（`buyUsd − sellUsd`）是全市场最大

两处**必须同口径**。此前混用钳位的 `weightedUsd` 与不钳位的 `signedNetUsd`，在「部分认输、部分还在买」这种常态场景下会互相打架 —— 纯二元市场即可构造出主导边判据误杀合法 C2 候选的反例（而二元市场是 Polymarket 主流）。
