...
- **已结算市场不再产生新的 `consensus`**：赎回在 Polymarket 走 `REDEEM`，不进
  成交流水，所以市场一结算，「谁还持有多少」就再也算不准（实测 73.6% 的市场在
  6h 检测窗口内结算，结算后中位 1.8 分钟就赎完）。引擎在推送前过一道结算闸门，
  已结算市场整组不发——客户端不必自己拿 `closed` 再筛一遍。注意这只管**新**信号：
  闸门上线前已入库的历史条目不会追溯删除
