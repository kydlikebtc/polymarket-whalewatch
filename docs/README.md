...
| `docs/plans/`                     | 设计文档与实现计划（24 份，按日期命名） | 想理解「为什么这么做」的人 | **刻意不更新**，是提案当时的快照 |
...
| 文件                  | 定位     | 读者       | 首次提交   | 最近改动   |
| --------------------- | -------- | ---------- | ---------- | ---------- |
| `docs/api-access.md`  | 对外使用 | 订阅方     | 2026-08-17 | 2026-08-18 |
| `docs/signals-api.md` | 内部契约 | 接口维护者 | 2026-07-29 | 2026-08-18 |

---

## 设计文档索引

`docs/plans/` 共 **24 份**，按日期倒序。同一主题的 design + implementation 相邻。
...
> 计数说明：`docs/plans/` 下 24 份 markdown = 17 份 `-design.md` + 3 份 `-implementation.md` + 4 份无后缀（`follow-strategy-tiers` / `follow-page-card-redesign` / `reverse-control` / `x-broadcast-bot`，正文自述为 Implementation Plan，上表已按实际类型标注）。连同 `docs/api-access.md`、`docs/signals-api.md` 两份对外文档与本索引，`docs/` 全目录共 27 份 markdown。
