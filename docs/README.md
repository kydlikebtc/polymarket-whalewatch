...
| `docs/plans/`                     | 设计文档与实现计划（35 份，按日期命名） | 想理解「为什么这么做」的人 | **刻意不更新**，是提案当时的快照 |
...
`docs/plans/` 共 **35 份**，按日期倒序。同一主题的 design + implementation 相邻。
...
| 2026-08-21 | [市场深度卡对外 API](plans/2026-08-21-market-card-api-design.md)                         | design         | 已有市场信号卡分层（marketBrief/marketCard）只缺对外形态：升级为带 key 的实时端点，服务订阅方用户「下单前的那一眼」；快照卡方案被否决（过时盘面答不了「现在该不该进」），L2 缓存表实现时按 YAGNI 砍掉。                              |
| 2026-08-21 | [市场深度卡对外 API](plans/2026-08-21-market-card-api.md)                                | implementation | 7 任务 TDD：进程内 24h 成交窗口增量续抓（冷启 1–13 请求、热续恒 1），全局令牌桶从属引擎健康度（staleLoops 非空即归零）；预算耗尽用陈旧窗口重算并标 `staleSec`，超 90s 硬闸 429——永不挤占引擎 data-api 预算。                         |
| 2026-08-19 | [X 推文文案 v2](plans/2026-08-19-x-post-copy-density-design.md)                          | design         | 蓝V只加发布长度不加折叠位：五类帖从实测 148~189 字符填满到 ≤280 加权字符（逐钱包胜率回执/时间集中度/cashtag/承诺行诚实闸门），`weightedLength` 统一加权口径，`fitPost` 先丢可选行、最后才截标题；否决「Fade or follow?」式提问结尾。 |
| 2026-08-19 | [X 推文文案 v2](plans/2026-08-19-x-post-copy-density.md)                                 | implementation | 12 任务 TDD（每任务一提交）：改动锁在纯函数模板层 `xComposer` 与三个消费循环的传参处，TG 主链路零改动；与设计的一处已论证偏差——战报佐证行用 `posted 2d ago` 而非 "settled today"（无真实结算时刻，说 today 会撒谎）。                |
...
| 2026-07-10 | [战略规划与迭代总纲](plans/2026-07-10-polymarket-monitor-strategic-roadmap.md)           | 战略总纲       | 12 个月规划（Living Document）：不再平铺更多看板与探测器，做三次升级——成交→统一市场级信号→可执行机会（反方证据/订单簿容量/滑点/信号年龄）→可验证策略（不可变快照+样本外回测），定位为「证明 edge 是否真的存在」。                    |
...
> 计数说明：`docs/plans/` 下 35 份 markdown = 24 份 `-design.md` + 3 份 `-implementation.md` + 8 份无后缀（其中 `2026-07-10-polymarket-monitor-strategic-roadmap` 是战略总纲、`2026-08-27-iteration-brainstorm-round2` 是脑暴快照；`follow-strategy-tiers` / `follow-page-card-redesign` / `reverse-control` / `x-broadcast-bot` / `x-post-copy-density` / `market-card-api` 正文自述为 Implementation Plan，上表按实际类型标注）。连同 `docs/api-access.md`、`docs/signals-api.md` 两份对外文档与本索引，`docs/` 全目录共 38 份 markdown。
