...
| `docs/plans/`                     | 设计文档与实现计划（48 份，按日期命名） | 想理解「为什么这么做」的人 | **刻意不更新**，是提案当时的快照 |
...
`docs/plans/` 共 **48 份**，按日期倒序。同一主题的 design + implementation 相邻。

| 日期       | 主题                                                                                     | 类型           | 一句话                                                                                                                                                                                                                                                                                       |
| ---------- | ---------------------------------------------------------------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-08 | [增量窗口维护与共识提频](plans/2026-09-08-incremental-window-design.md)                 | design         | formation→emitted 实测中位 ~329s 的主项是 5 分钟轮询,而周期被「每轮全量重拉 6h 窗口(98.6% 重复、轮末即弃)」钉死——windowKeeper 常驻 6h 缓冲、每 90s 只抓水位线增量,抓取窗口与分析窗口解耦(分析 6h 是信号定义红线);净买账完整性 fail-closed(不衔接即整轮丢弃+全量重扫)、30k 行数上限、config 一键回滚。 |
| 2026-09-07 | [产品迭代脑暴 · 第四轮](plans/2026-09-07-iteration-brainstorm-round4.md) | 脑暴快照 | 从九个代码/运营实测事实出发的 47 个净新增方向（27 + 同日补遗 20）（四个透镜：连续性保险 / 「拉」式出口 / 闸门后第二幕 / 采了没问的数据，外加一条有界成交缓存红线重议与 AI 出口第二层）；第一梯队六条零上游「补工具护资产」（上游计量表 / market_daily 心跳 / 异地备份 / 参数进存证链 / 公开验证器 / RSS）；文末附五档必要性排序与四周节奏。 |
| 2026-08-31 | [兑现事件对外一等化](plans/2026-08-31-settle-event-parity-design.md)                     | design         | 拉/推同构的策略动作流：推送侧本就有 entry/settle 两种事件，拉取侧却只有状态视图（active 结算后行消失、settled 是 3d/20 条战绩），只轮询的订阅方看不到兑现——新增 strategies.events[]，逐条 = webhook SignalEventV1（同一构造器产出），名录同步声明事件种类；不动数据模型、不动既有视图。      |
| 2026-08-28 | [功能说明书 /guide](plans/2026-08-28-feature-guide-design.md)                            | design         | 全站板块总说明书：每板块「是什么 · 怎么用 · 怎么读」，read 块继承诚实纪律（每节必带口径与「别这么读」）；glossary 数据模式（中文唯一源 + 渲染处 t()），NAV/sitemap ⊆ 说明书的防漏测试；页面公开但不进导航，入口先挂 /manage 双位（复刻 /status 先例）。                                      |
| 2026-08-28 | [聪明钱自测](plans/2026-08-28-smart-money-selftest-design.md)                            | design         | 访客粘贴地址领判决书：严格复用 evaluateAdmission 战绩闸（复发广度是发现渠道专属，不适用）+ 池内三轴 midrank 分位 + 可分享嵌入卡；成本闸为上线前置——24h 判决缓存、与档案页同桶限流、限流/故障回本地判决、嵌入卡零上游红线；判决「没过」与「判不了」严格分家。                                 |
| 2026-08-28 | [第二梯队八件套](plans/2026-08-28-tier2-octet-design.md)                                 | design         | 第三轮脑暴第二梯队全量同批设计：价格影响持久性（price_10m 列 + 钱包级留存率）、时光机（点击加载的市场复盘曲线）、离场卖侧窗口、池内行为指纹（可解释规则拒绝黑盒聚类）、名人堂+反指名单（逐钱包 CRVE 同纪律 + 多重比较披露）、无鲸异动与洗量（market_daily 两列）；六件零上游、两件有界例外。 |
| 2026-08-28 | [第一梯队五件套](plans/2026-08-28-tier1-quintet-design.md)                               | design         | 第三轮脑暴裁决的五条零新增上游组同批设计：容量标尺（+1¢/+3¢ 带内深度，归因列红线）、确信指数（品类×日四分量，读取侧现算）、衰变哨兵（市场级聚簇 CUSUM 四态）、重推日历化（90 天 due + 报告 diff，不自动跑）、同批出生（深窗口第五消费者，年龄严格缓存裸读，TG 默认关）。                     |
| 2026-08-28 | [产品迭代脑暴 · 第三轮](plans/2026-08-28-iteration-brainstorm-round3.md)                 | 脑暴快照       | 17 个净新增方向（五组：微观结构与归因 / 钱包作为实体 / 访客进实验 / 市场风险层 / 自我免疫），由闸门达标 + 向前落库 + 订单簿快照三件事解锁；附性能×存储成本评估——第一梯队五条零新增上游一两周可上，红线是自由式策略工坊与归档成交流水。                                                       |
| 2026-08-28 | [渠道效果记分卡](plans/2026-08-28-channel-scorecard-design.md)                           | design         | 发现渠道跑了近两个月首次验收：告警台账天然是逐钱包前向实验（smart 单钱包/consensus 成员展开），按首发渠道 source 分组算净 edge ± 聚类 CI；离池钱包设「来源失联」独立桶防幸存者偏差；MM 横切直答 72 机器人悬案；只展示不接清退。                                                              |
| 2026-08-28 | [信号行向前落库](plans/2026-08-28-signal-forward-facts-design.md)                        | design         | walk-forward 点名的 v2 前置：strategy_signals 补 wallets_json（触发钱包+彼时评分+逐钱包金额，纯归因不参与开仓判定）——score 维与真逐钱包阈值只能向前记无法回填，晚一天=永久少一天可回放窗口；对外面逐处核对为显式投影，零消费方改动+惰性守卫。                                                |
...
| 2026-08-21 | [市场深度卡对外 API](plans/2026-08-21-market-card-api-design.md)                         | design         | 给「正要下单」的交易者一张带 key 的实时市场深度卡（谁在买/多强/成本/有无分歧/历史信号准不准）；快照卡方案被否决——交易决策要的是此刻；真正的技术问题是原限流闸门装错维度：限的是请求数，而成本取决于去重后的市场数。                                                                          |
| 2026-08-21 | [市场深度卡对外 API](plans/2026-08-21-market-card-api.md)                                | implementation | 进程内 24h 成交窗口只增量续抓（冷启 1–13 请求、热续恒 1），全局令牌桶计量续抓次数且额度从属引擎健康度；预算耗尽用陈旧窗口重算并标 `staleSec`，超 90s 硬闸 429；设计中的 L2 缓存表按 YAGNI 砍掉。                                                                                             |
| 2026-08-19 | [X 推文文案 v2](plans/2026-08-19-x-post-copy-density-design.md)                          | design         | 蓝V提升的是可发布长度、不改 ≈280 加权字符的时间线折叠位，实测现有模板只用 148–189 字符——把空额填满（钱包胜率回执/时间集中度/赛前双边资金），X 按条计费、加长免费；底座是 `weightedLength` 加权口径（emoji 计 2）与 `fitPost` 降级阶梯（先丢可选行、最后才截标题）。                          |
| 2026-08-19 | [X 推文文案 v2](plans/2026-08-19-x-post-copy-density.md)                                 | implementation | 全部改动收在纯函数模板层 `lib/xComposer.ts` 与三个消费循环的传参处，新增 `weightedLength` / `fitPost` 两个纯函数底座；凭证数据走本地 SQLite 零上游请求，不改任何 TG 主链路代码。                                                                                                             |
...
| 2026-07-10 | [战略规划与迭代总纲](plans/2026-07-10-polymarket-monitor-strategic-roadmap.md)           | roadmap        | 12 个月规划总纲（Living Document 提案版）：三次升级路线——从成交到信号、从信号到可执行机会、从机会到可验证策略；并列出六项不参与普通优先级竞争的 P0 可信度债务（净仓位口径、结果回填完整性、采集与投递解耦等）。                                                                              |
...
> 计数说明：`docs/plans/` 下 48 份 markdown = 34 份 `-design.md` + 3 份 `-implementation.md` + 11 份无后缀（其中 `2026-07-10-polymarket-monitor-strategic-roadmap` 是战略总纲、`2026-08-27-iteration-brainstorm-round2`、`2026-08-28-iteration-brainstorm-round3` 与 `2026-09-07-iteration-brainstorm-round4` 是脑暴快照；`follow-strategy-tiers` / `follow-page-card-redesign` / `reverse-control` / `x-broadcast-bot` / `x-post-copy-density` / `market-card-api` / `walkforward-rederivation` 正文自述为 Implementation Plan，上表按实际类型标注）。连同 `docs/api-access.md`、`docs/signals-api.md` 两份对外文档与本索引，`docs/` 全目录共 51 份 markdown。
