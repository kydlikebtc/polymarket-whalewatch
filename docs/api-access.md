...
「你的 key **允许**拿什么」和「服务端此刻**真的在产出**什么」是两件事。
下表是后者，**由本页在渲染时读取当前开关实时生成**，不是手写快照：
...
- 本文其余部分描述的是**系统能力的全集**——字段契约、判据、类型都不随开关
  变化。上表只回答「这些能力此刻有几个是开着的」。
- **`strategies` 一栏列出的档位就是你能收到的全部档位。** §6.7.2 的 19 档
  名录是系统支持的全集（它们都在跑纸面仓），但对外发布由运营者逐档放开。
  **请始终以 `recordByStrategy` 的键为准**遍历档位，不要按名录写死档数——
  运营者放开更多档时你的代码应当自动适配。
- **总线类型标「未开启」时，`bus[]` 里不会有该类型的任何条目。** 三类全关
  时 `bus` 恒为 `[]`——这不是故障，也不是你接错了。
- 想调整任何一项，联系运营者。
...
先立一个判据：**触发后发出的才是信号（事件）**——不可变、逐条、有稳定 id，
推送与计数都以它为准。据此本服务只有**两条信号线**，其余字段是**视图**
（事件的折叠/汇总，回答「现在该看什么」，非信号、不承担触发语义）。

**信号（事件）：**

| 信号线         | 事件类型                                    | 在响应的哪里                            | 订阅范围   |
| -------------- | ------------------------------------------- | --------------------------------------- | ---------- |
| **① 原始事件** | 大额成交 / 聪明钱共识 / 钱包发现（+待接入） | `bus[]`（§6.8）                         | 逐类型     |
| **② 策略事件** | 19 档纸面策略的买入 / 结算                  | `strategies.active` / `settled`（§6.7） | `strategy` |

**视图（非信号）：**

| 视图                     | 由什么折叠而来                     | 在哪一节  |
| ------------------------ | ---------------------------------- | --------- |
| `active[]` / `settled[]` | ① 的大额/共识事件，按市场×方向折叠 | §6.4/§6.5 |
| `record30d`              | ① 已结算事件的 30 天价格调整汇总   | §6.6      |
| `recordByStrategy`       | ② 事件的按档 30 天汇总             | §6.7.4    |

三条使用纪律：

- **原始事件是原料，策略事件是成品。** ① 说「有 3 个聪明钱买了 Yes」；② 说
  「基于这类事实，某档在 0.63 建了 $500 纸面仓」——只有成品有战绩。
- **原始事件是流水，不是判断。** `large` 只说「有人砸了一笔大单」，不含任何
  「值得跟」的意思（实测 notional < $25k 占全部告警的 69%）。
- **不要跨线/跨形态相加。** 同一笔市场行为可能同时出现在 `bus[]`、
  `active[]` 与多个档位的 `strategies.active[]` 里——同一事实的多个视角。

> ⚠️ **战绩只属于策略线。** `record30d` 是聪明钱动向的战绩，
> `recordByStrategy` 是各档策略的战绩，两者口径不同、**不可比也不可加**。
> 详见 §6.6 与 §6.7.4。

**折叠视图 `active[]` 与事件 `bus[]` 怎么选**——同一批 ① 事件的两种形态：

- **③ 是事件**：回答「刚刚发生了什么」。不可变、逐条、有稳定 id，因此可
  推送（webhook 的 at-least-once 依赖这一点）；
- **① 是状态**：回答「现在该看什么」。按市场×方向折叠、随共识升级**原地
  更新**、带战绩——它是给页面渲染的快照，不是触发源。

两者读同一张底层台账，**事实上不可能互相矛盾**；同时消费也不会双写——
③ 的开关只控制「是否投影成可推送的事件台账」，① 恒产出、不受影响。
真正的坑是**重复计数**：同一共识组每次升级（2 人 → 3 人）在 ③ 里是一条
**新事件**（`dedupKey` 不同，`walletCount` 变大），把每条事件当一个独立
共识，一个组就会被数成多个；① 已替你折叠成一条。

推荐消费模式：**③ 做触发，① 做渲染**——webhook 事件到达后，拿
`conditionId` + `outcome` 去 ① 取当前折叠状态展示。只用 ① 轮询并自己
diff `key` 集合也能当触发（③ 出现之前的唯一办法），但延迟受轮询间隔限制。

维度对照：

| 维度 | ① `active[]`/`settled[]`                        | ③ `bus[]`                               |
| ---- | ----------------------------------------------- | --------------------------------------- |
| 粒度 | 按市场×方向**折叠**（同一仓位只出一条）         | **逐事件**流水                          |
| 规则 | 固定（≥2 钱包 / 两侧 / 单笔 ≥$50k，白名单口径） | 运营者阈值，含非白名单大额与钱包发现    |
| 形态 | 拉取快照（升级会原地更新金额）                  | 不可变台账（有稳定 id，因此**可推送**） |
| 用途 | 渲染卡片、看战绩                                | 事件驱动、webhook 消费                  |

想经 **webhook** 接收聪明钱动向：订阅总线的 `consensus` / `large` 类型
（§10）——①本身是折叠视图，没有稳定的逐事件 id，不适合作为推送事件；它的
推送形态就是 ③。注意 ③ 的 `large` 同时包含白名单与非白名单成交（payload
带 `wallet` 但不带白名单标记），比 ① 的 `heavy` 口径更宽。
...
**只包含运营者已放开推送的档位**——当前放开了哪几档见 §4.1 的实时状态表。
**遍历档位请用 `recordByStrategy` 的键**，不要按 §6.7.2 的名录写死档数：
运营者放开更多档时你的代码应当自动适配。
...
> **这 19 档都在跑纸面仓，但不等于都会发给你**——对外发布由运营者逐档放开，
> 当前放开了哪几档见 §4.1 的实时状态表，代码里请以 `recordByStrategy` 的键
> 为准遍历。
>
...
> ⛔ **先看 §4.1 的实时状态表再读本节。** 总线的三个类型各自有开关，标为
> 「未开启」的类型不会在 `bus[]` 里产生任何条目；三类全关时 `bus` 恒为 `[]`
> ——那是预期行为，不是故障。本节描述的是类型**开启后**的形态，字段契约不随
> 开关变化。想启用请联系运营者。
...
> 同一类型可能配置了**多档阈值定义**（运营者侧概念，如「大额 ≥$50k /
> 巨额 ≥$500k」）。`bus[]` 拉取是**类型级**的——包含该类型全部入账事件
> （入账下限 = 各启用档的最小阈值）；按档过滤请用 payload 里的数值字段
> 自行筛，或让运营者把你的 webhook 端点配置成只订某一档。
...
> ⚠️ **同一共识组的每次升级都是一条新事件**（2 人成团一条、第 3 人加入
> 再一条，`walletCount` 递增）。事件语义是「共识状态变化」，不是「又一个
> 新共识」——统计共识**个数**请按 `(conditionId, outcome)` 归并，或直接
> 用 ① 的折叠结果（§6.1 的「③ 触发、① 渲染」模式）。
...
总线类型 `large` / `consensus` / `discovery`；① 的类型还可**按信号定义
细分订阅**——运营者可为同一类型配置多档阈值定义（例如「大额 ≥$50k」与
「巨额 ≥$500k」两档），你的端点可以只订其中一档（收到的事件体不变，只是
子集）。三条规则：
...
的推送。§4.1 的实时状态表会告诉你当前放开了哪几档；若那里显示有档位而你仍拿
不到，就是 key 的订阅范围问题，联系运营者。
...
| 日期       | 变更                                                                                                                                                                                                                                                                                   |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-19 | ① 事件类型支持**多档信号定义**（同类型不同阈值、各自启停），webhook 端点可按档订阅；`bus[]` 拉取保持类型级（入账下限=启用档最小阈值）                                                                                                                                                  |
| 2026-08-19 | §6.1 重构分类：信号=事件（两条线：原始事件/策略事件），`active[]`/`record30d` 等定性为**视图**（非信号、无触发语义）——「聪明钱动向」不再作为独立信号线出现                                                                                                                             |
| 2026-08-19 | §6.1 增「状态 vs 事件」框架与「③ 触发、① 渲染」消费模式；§6.8 明确共识升级会产生多条事件（防重复计数）                                                                                                                                                                                 |
| 2026-08-19 | §6.1 补「① 与 ③ 怎么选」对照表与 webhook 接收 ① 的路径说明（订阅总线 consensus/large；③ 的 large 口径比 ① 的 heavy 宽）                                                                                                                                                                |
