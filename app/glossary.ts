...
export const TERMS: TermEntry[] = [
  ...
  },
  {
    term: "净买入",
    detail:
      "按（钱包 · 市场 · 结果）聚合。共识/分歧/拆单/发现渠道的门槛与排序统一采用成本敞口口径：留存净股数（买入股数 − 卖出股数）× 买入均价——高买低卖等股对倒后真实仓位为 0，不再被'买入额 − 卖出额'的现金流差伪装成净买入。拆单榜的净买入列仍显示窗口现金流（买入额 − 卖出额）供对照。注意口径：拆单榜只统计 ≥ 精度 floor 的成交，低于 floor 的卖出在该精度下不可见——净买入应视为上界（表头'毛卖出(≥floor)'即此意）。",
  },
  {
    term: "对冲嫌疑",
    detail:
