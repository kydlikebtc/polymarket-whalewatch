...
export const TERMS: TermEntry[] = [
  ...
  },
  {
    term: "聪明钱发现渠道",
    detail:
      "白名单之外的聪明钱来源。官方榜单只按规模（盈利/成交额）排名，会系统性漏掉『有本事但资金不大』的钱包。发现页汇聚四条渠道的候选：🔁 共识同行（与白名单共识同一结果同向净买 ≥$2k）、🧩 拆单建仓（≥3 笔低于 $10k 的拆单净买 ≥$5k 且无对冲/做市嫌疑）、🕵️ 内幕签名（账龄 ≤7 天新地址单笔 ≥$5k 买 0.5–0.9 价带）、🎯 早期赢家（在已结算市场提前 ≥24 小时以 ≤40¢ 买中获胜结果——衡量技能而非规模）。另有 🏅 分类榜专家（整体平庸但称霸某赛道的钱包）经每日播种直接入池。",
  },
  {
    term: "准入闸门",
    detail:
      "候选进入白名单池的唯一通道。因为共识检测与 smartOnly 只看『钱包在不在池里』，不设闸门的发现渠道会悄悄改变所有下游信号的含义。闸门三关：①复发——30 天内在 ≥3 个不同市场留下证据（一个市场可能是运气，三个像一个过程）；②战绩审查——结算胜率 ≥55% 且 ≥10 个已结算市场，或净盈亏为正且 ROI ≥5%（刻意不用 0-100 评分做门槛：其利润轴会重新引入渠道要逃离的规模偏置）；③做市机器人（≥1000 市场）硬拒。入池后与榜单钱包同规则：30 天不再合格自动出池。每个成员的 source 字段记录首发渠道，为后续按渠道评估有效性留好归因。",
  },
  {
    term: "颜色语义",
    detail:
