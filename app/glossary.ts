...
export const TERMS: TermEntry[] = [
  ...
  },
  {
    term: "纸面跟单",
    detail:
      "不动真金的模拟跟单：聪明钱共识一旦形成，就以当时现价、按固定 $/信号纸面买入，持有到市场结算才平仓——只结算真实盈亏，期间不计浮盈。分歧市场（聪明钱两边都买）不跟——与共识/分歧看板的市场级互斥口径一致。用来诚实检验「跟着共识买」这套策略在本窗口到底赚不赚钱，而无需真的下注。/follow 看板每张策略卡即一组阈值（≥N 钱包 · 每钱包 ≥$X · $Y/信号）的独立纸面战绩。",
  },
  {
    term: "跟单滑点",
    detail:
      "份额 ×（自己入场价 − 聪明钱建仓均价）。共识形成后我们才现价跟进：入场价高于均价（追高）时为正，是多付出的成本、拖累收益。⚠️ 负值并不等于「买得更便宜」——当入场价远低于聪明钱均价，往往是行情在其建仓后已大幅反向、共识论点可能已被推翻（接飞刀），务必结合「已实现盈亏」一起看。入场价越低份额越膨胀，滑点的美元绝对值可能超过本金，因此更宜看「¢ 差」而非美元金额——看板即按 ¢ 差主显示，美元值退居次要。策略默认带 10¢ 进场偏离护栏：现价偏离聪明钱均价超阈直接不跟，宁可错过也不追高 / 接飞刀。累计滑点把每一仓（含持仓中）的差价加总。",
  },
  {
    term: "净值曲线",
    detail:
