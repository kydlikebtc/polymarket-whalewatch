...
```text
   ...
   user-pnl-api                    Polygon public RPC (PUSD balance)
     ...
     ║                                      ║        │  (/api/scan, /consensus,   │
     ║  alert 4s ── consensus 90s            ║        │   /accumulation, /market,  │
...
| `consensus`         | 30s       | 90s                                           | One 6-hour, $2k-floor trade window (windowKeeper: resident buffer + incremental fetch, hourly full resweep) shared by four consumers: consensus detection + push, firehose evidence collection, `runFollowCycle` (all 19 strategies open/settle), and `projectBusSignals`.                                                                                                             | `consensus`        |
