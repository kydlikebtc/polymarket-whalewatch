...
export const DICT_GLOSSARY: Record<string, string> = {
  ...
  "发现页的 14 天趋势表，覆盖两个信号面：左侧列跟踪共识信号引擎——每日共识推送数 ÷ 当日平均 6 小时窗口成交量（按 $1M 归一）；『新证据』列跟踪发现渠道——四条渠道（共识同行/拆单建仓/内幕签名/早期赢家）当日首次入账的候选证据行数。所有共识/拆单阈值（2 钱包 / $5k / 6h …）都隐含『当前成交量水位』假设——预测市场热度是周期性的，大盘量退潮后要么信号荒（共识永不成组）要么被迫降阈值引入噪声。密度随热度同跌 = 市场自身降温；热度稳定而密度独跌 = 阈值漂移需要重校。注意窗口每 90 秒滚动、彼此重叠，成交量不能跨轮求和，取当日平均窗口量作为热度代理。":
    "The discovery page's 14-day trend table, covering two signal surfaces. The left columns track the consensus signal engine — daily consensus pushes ÷ that day's average 6-hour-window volume (normalized per $1M). The 'new evidence' column tracks the discovery channels — candidate evidence rows first recorded that day across the four channels (consensus echo / split-buy entry / insider signature / early winner). Every consensus/split-buy threshold (2 wallets / $5k / 6h …) implicitly assumes 'the current volume level' — prediction-market heat is cyclical, and when overall volume ebbs you get either a signal drought (consensus never forms) or forced threshold cuts that let in noise. Density falling with heat = the market itself cooling; density falling alone while heat holds = threshold drift needing recalibration. Note the windows roll every 90 seconds and overlap, so volume cannot be summed across cycles; the day's average window volume serves as the heat proxy.",
