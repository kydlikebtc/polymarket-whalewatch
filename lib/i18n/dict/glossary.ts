...
export const DICT_GLOSSARY: Record<string, string> = {
  ...
  "验证闭环：每条告警（含 🔥 共识，按成员加权买入均价与最后一笔成交时间跟踪）自动回填信号发出后 1 小时 / 24 小时的市场价格变化（按方向着色，±0.5¢ 死区内记平推）与最终结算结果。页面顶部按信号类型（💰大单 / 🏆聪明钱 / 🔥共识）分组汇总 1h/24h 方向命中率和已结算胜率，样本 ≥10 时附 Wilson 95% 区间，样本不足时置灰提示——让工具为自己的信号打分。数据按需查询公开历史价格，不依赖归档。":
    "The validation loop: every alert (including 🔥 consensus, tracked by member-weighted average entry price and last-fill time) automatically backfills the market's price change 1 hour / 24 hours after the signal (direction-colored; flat within the ±0.5¢ dead zone) and the final settlement result. The top of the page aggregates 1h/24h direction hit rates and settled win rates by signal type (💰 large trades / 🏆 smart money / 🔥 consensus), with a Wilson 95% interval when the sample is ≥10 and a grayed-out note when it is too small — letting the tool grade its own signals. Data is fetched on demand from public price history; no archival required.",
  // ↗
  外部跳转: "External links",
  ...
  "Telegram 推送里的『剔除运气后至少 X%』= 命中率的 Wilson 95% 置信下界：小样本下表面命中率极不可靠——3/3 显示 100%，剔除运气后其实只能保证约 44%；同样 65% 的表面命中率，26 个样本只能保证 46%，260 个样本能保证 59%。样本越多、下界越接近表面值。宁可把话说小：展示的是有 95% 把握守得住的下限，不是最好看的点估计。样本 <10 时看板汇总条置灰标'样本不足'（推送 <5 时同理），因为那时连下界都没有统计意义。":
    "'At least X% net of luck' in the Telegram pushes = the Wilson 95% confidence lower bound of the hit rate. On small samples the surface hit rate is wildly unreliable — 3/3 reads 100%, yet net of luck only about 44% can be guaranteed; the same 65% surface rate guarantees just 46% on 26 samples but 59% on 260. The more samples, the closer the bound gets to the surface value. Better to understate: what is shown is the floor we are 95% confident of holding, not the prettiest point estimate. Below 10 samples the dashboard summary strip grays out as 'insufficient sample' (pushes do the same below 5), because at that point even the lower bound has no statistical meaning.",
